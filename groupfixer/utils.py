...
DEFAULT_MINIMUM_FEMALE_PROPORTION = 0.0
DEFAULT_MAXIMUM_FEMALE_PROPORTION = 1.0
...
def get_female_proportion_status(
    member_list, minimum_female_proportion, maximum_female_proportion
):
    m = 0
    f = 0
...
def print_diagnostics(groups, members, constraints):
    ...
    minimum_female_proportion = constraints.get(
        "min_female", DEFAULT_MINIMUM_FEMALE_PROPORTION
    )
    maximum_female_proportion = constraints.get(
        "max_female", DEFAULT_MAXIMUM_FEMALE_PROPORTION
    )

    diag = ""
    for i in range(len(groups)):
        size_status = get_size_status(members[i], minimum_size, maximum_size)
        gender_status = get_female_proportion_status(
            members[i], minimum_female_proportion, maximum_female_proportion
        )
        diag += "{}:\t{} members{}\t{}\n".format(
            groups[i].name, len(members[i]), size_status, gender_status
        )
    return diag
...
def run_assign_groups(constraints):
    ...
    respect_non_alcoholic = constraints.get(
        "respect_non_alcoholic", DEFAULT_RESPECT_NONALCOHOLIC
    )
    ...
    count_groups_matrix = np.repeat(
        np.identity(number_of_users), number_of_groups, axis=1
    )
    ...
    for i in range(number_of_users):
        if users[i].gender == "other":
            count_female_matrix[
                :, i * number_of_groups : (i + 1) * number_of_groups
            ] = zeroing_matrix
            count_male_and_female_matrix[
                :, i * number_of_groups : (i + 1) * number_of_groups
            ] = zeroing_matrix
    ...
    one_group_constraint = count_groups_matrix @ placement_vector == np.ones(
        number_of_users
    )
    non_alcoholic_constraint = (
        1 - non_alcoholic_vector
    ) * placement_vector == np.zeros(len(non_alcoholic_vector))

    max_users_in_group_constraint = count_users_matrix @ placement_vector <= np.full(
        number_of_groups, maximum_size
    )
    min_users_in_group_constraint = count_users_matrix @ placement_vector >= np.full(
        number_of_groups, minimum_size
    )

    count_male_and_female_matrix = count_male_and_female_matrix

    min_female_proportion_constraint = (
        count_female_matrix @ placement_vector
        >= minimum_female_proportion * count_male_and_female_matrix @ placement_vector
    )

    max_female_proportion_constraint = (
        count_female_matrix @ placement_vector
        <= maximum_female_proportion * count_male_and_female_matrix @ placement_vector
    )
    ...
    if respect_non_alcoholic:
        problem = cvxpy.Problem(
            objective,
            [
                one_group_constraint,
                non_alcoholic_constraint,
                max_users_in_group_constraint,
                min_users_in_group_constraint,
                min_female_proportion_constraint,
                max_female_proportion_constraint,
            ],
        )
